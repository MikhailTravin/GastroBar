...
export function formQuantity() {
	// Функция для обновления состояния кнопки на основе значения
		...
		if (quantityControl) {
			...
			if (targetElement.hasAttribute('data-quantity-plus')) {
				...
				if (+valueElement.dataset.quantityMin) {
					...
					value = 0; // Устанавливаем 0, чтобы триггернуть удаление
				}
			}

			valueElement.value = value;

			// Если значение <= 0 и есть ближайший .order__column — удаляем его
			if (value <= 0) {
				const orderColumn = quantityBlock.closest('.order__column');

				if (orderColumn) {
					orderColumn.classList.add('fade-out'); // Запускаем анимацию
/*
					// Ждём завершения анимации и скрываем элемент
					setTimeout(() => {
						orderColumn.style.display = 'none';
					}, 100); // 300 мс — длительность вашей анимации*/
				}
			} else {
				// Обновляем состояние кнопки только если значение положительное
